...
function finishQuiz(){
  ...
  if(exam){
    ...
    $("#resDetail").textContent = round1(earned)+T(" von "," of ")+maxpts+T(" Punkten · Zeit: "," points · time: ")+fmtTime(usedMs)+T(" min · 60 % nötig (offizielle iSAQB-Bewertung).", " min · 60 % required (official iSAQB scoring).")+(mock?T(" · offizielle iSAQB-Beispielprüfung"," · official iSAQB example exam"):"");
...
function renderStats(){
  ...
  else { const p=Math.round(ec/ans*100); const v=p>=60?"<span class='pill ok'>"+T("wahrscheinlich bestanden","likely to pass")+"</span>":(p>=50?"<span class='pill mid'>"+T("grenzwertig","borderline")+"</span>":"<span class='pill no'>"+T("noch nicht bereit","not ready yet")+"</span>");
    rd.innerHTML="<div style='font-size:2.2rem;font-weight:800'>"+p+"%</div><div class='lbl'>"+T("Trefferquote auf R1/R2 (","Accuracy on R1/R2 (")+ans+T(" beantwortet) &nbsp;"," answered) &nbsp;")+v+"</div><p class='hint'>"+T("Grobe, eher konservative Schätzung (zählt nur komplett richtige Antworten; die echte Prüfung vergibt auch Teilpunkte). 60 % zum Bestehen.","Rough, rather conservative estimate (counts only fully correct answers; the real exam also awards partial points). 60 % to pass.")+"</p>"; }
